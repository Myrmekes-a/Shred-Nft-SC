...
impl UserPool {
    pub fn add_nft(&mut self, nft_pubkey: Pubkey, is_legendary: u8, now: i64) {
    ...
    pub fn remove_nft(&mut self, nft_pubkey: Pubkey, now: i64) -> Result<u64> {
        ...
        } else if self.staked_mints[index].is_legendary == 2 {
            // diamond can't upgrade for rebirth
            reward_amount = DIAMOND_REWARD_AMOUNT;
        ...
        let reward =
            (((now - last_reward_time) / EPOCH) as u64) * reward_amount * factor / 100 as u64;
