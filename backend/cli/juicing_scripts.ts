...
import { GlobalPool, NftPool } from './juicing_types';
...
const juicingIdl = JSON.parse(
    fs.readFileSync(__dirname + "/../target/idl/juiced_ape_evolution.json", "utf8")
);
...
const main = async () => {
    ...
    console.log("globalPool =", globalPool.superAdmin.toBase58(), globalPool.juicingFeeWhey.toNumber(), globalPool.juicingFeeSol.toNumber(), globalPool.totalJuicedCount.toNumber(), globalPool.totalMutedCount.toNumber());
...
export const initProject = async (
    ...
    const tx = await juicingProgram.rpc.initialize(
        bump, null, null, null, {
        accounts: {
...
//     const tx = await juicingProgram.rpc.initializeNftPool(
//         bump, {
//         accounts: {
//             owner: userAddress,
//             nftMint: mint,
...
//         programId: juicingProgram.programId,
//     });
...
// export const stakeNft = async (userAddress: PublicKey, mint: PublicKey, isLegendary: boolean, tier: number) => {
...
//     let burnVault = await getAssociatedTokenAccount(BURN_WALLET_ADDRESS, REWARD_TOKEN_MINT);
//     console.log("Burn Token Account = ", burnVault.toBase58());

//     const tx = await program.rpc.stakeNftToPool(
//         bump, isLegendary, tier, {
//         accounts: {
//             owner: userAddress,
//             userPool: userPoolKey,
//             globalAuthority,
...
//         }
//         let reward = 0;
//         reward = (Math.floor((now - lastRewardTime) / EPOCH)) * rewardAmount * factor / 10000;
...
export const getGlobalState = async (
    ...
    try {
        let globalState = await juicingProgram.account.globalPool.fetch(globalAuthority);
